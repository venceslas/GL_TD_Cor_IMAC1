...
namespace glbasimac {
	...
	void GLBI_Engine::initGL() {
		...
		if (mode2D) {
			...
			idShader[0] = ShaderManager::loadShader(std::string(PROJECT_ROOT) + "assets/shaders/flat_shading_2D.vert",std::string(PROJECT_ROOT) + "assets/shaders/flat_shading.frag",true);
		}
		else {
