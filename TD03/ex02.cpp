...
static const float GL_VIEW_SIZE = 180.;
...
void renderScene() {
	...
	anim_angle += 0;//anim_sens;
	if ((anim_angle >= 45) || (anim_angle <= -45.)) {
		anim_sens *= -1;
	}  

	myEngine.mvMatrixStack.pushMatrix();
		myEngine.mvMatrixStack.addTranslation({-60.0,0.0,0.0});
		myEngine.mvMatrixStack.addRotation(deg2rad(anim_angle),{0.0,0.0,1.0});
		drawFirstArm();
		myEngine.mvMatrixStack.pushMatrix();
			myEngine.mvMatrixStack.addTranslation({60.0,0.0,0.0});
			myEngine.mvMatrixStack.addRotation(-deg2rad(angle_beta),{0.0,0.0,1.0});
			drawSecondArm();
			myEngine.mvMatrixStack.pushMatrix();
				myEngine.mvMatrixStack.addTranslation({40.0,0.0,0.0});
				myEngine.mvMatrixStack.pushMatrix();
					myEngine.mvMatrixStack.addRotation(deg2rad(45.),{0.0,0.0,1.0});
					drawThirdArm();
				myEngine.mvMatrixStack.popMatrix();
				myEngine.mvMatrixStack.pushMatrix();
					myEngine.mvMatrixStack.addRotation(deg2rad(0.),{0.0,0.0,1.0});
					drawThirdArm();
				myEngine.mvMatrixStack.popMatrix();
				myEngine.mvMatrixStack.pushMatrix();
					myEngine.mvMatrixStack.addRotation(deg2rad(-45.),{0.0,0.0,1.0});
