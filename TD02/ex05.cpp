...
void renderScene() {
	...
	Vector3D tr{0.5*cos(M_PI/4.0),0.5*sin(M_PI/4.0),0.0};
	myEngine.mvMatrixStack.addTranslation(tr);
	myEngine.mvMatrixStack.addRotation(M_PI/3,Vector3D(0.,0.,1.0));
	myEngine.mvMatrixStack.addHomothety(Vector3D(0.5,0.5,1.0));
	myEngine.updateMvMatrix();
	myEngine.setFlatColor(1.0,0.0,0.0);
	triangle.drawShape();
	myEngine.mvMatrixStack.loadIdentity();

	Vector3D tr2{-0.5*cos(M_PI/4.0),0.5*sin(M_PI/4.0),0.0};
	...
	float r{0.25};
	float ang{M_PI/4.};
	myEngine.mvMatrixStack.addTranslation({r*cosf(ang),r*sinf(ang),0.0});
	myEngine.mvMatrixStack.addHomothety(0.2);
	myEngine.updateMvMatrix();
